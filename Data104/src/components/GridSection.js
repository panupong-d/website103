...
const GridSection = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://charoenkrung103api-ca7c8d01cc0e.herokuapp.com/data103/id_all');
  ...
  const handleCardClick = async (id) => {
    try {
      const response = await fetch(`https://charoenkrung103api-ca7c8d01cc0e.herokuapp.com/data103/id/${id}`);
      if (!response.ok) throw new Error('Network response was not ok');
